...
def run_blastx(query_nt_fpath, target_prot_fpath, db_name, out_fname):
...
def main() -> int:
    query_accession_dir, target_accession_dir, job_id = sys.argv[1:4]

    query_accession = os.path.basename(query_accession_dir)
    target_accession = os.path.basename(target_accession_dir)

    query_fasta = os.path.join(query_accession_dir, f"{query_accession}.fna")
    query_gff = os.path.join(query_accession_dir, f"{query_accession}.gff")
    target_fasta = os.path.join(target_accession_dir, f"{target_accession}.fna")
    target_gff = os.path.join(target_accession_dir, f"{target_accession}.gff")

    # CDS files
    query_cds = os.path.join(query_accession_dir, f"{query_accession}_cds.fna")
    target_cds = os.path.join(target_accession_dir, f"{target_accession}_cds.fna")
    target_cds_prot = os.path.join(target_accession_dir, f"{target_accession}_cds.faa")

    # Temp job-specific files
    blast_db_name = f"{job_id}_target_db"
    blast_results = f"{job_id}_results.xml"

    results = None

    try:
        print(f"Job {job_id}: Query={query_accession}, Target={target_accession}", file=sys.stderr)

        # extract CDS if it does not exist
        if not os.path.exists(query_cds):
            extract_cds(query_fasta, query_gff, query_cds, job_id)

        if not os.path.exists(target_cds):
            extract_cds(target_fasta, target_gff, target_cds, job_id)

        if not os.path.exists(target_cds_prot):
            nt_to_prot(target_cds, target_cds_prot)

        results = run_blastx(query_cds, target_cds_prot, blast_db_name, blast_results)
        ...
        for f in files_to_remove:
            try:
                ...
                print(f"Warning: Failed to remove {f}: {e}", file=sys.stderr)
